function expressionCalculator(expr) {
  ...
  for (let i = 0; i < inputArr.length; i++) {
    if (+inputArr[i] >= 0) exprToRpn.push(+inputArr[i]);
      ...
      while (stack[stack.length - 1] != '(') {
        if (stack.length < 1) throw new Error('Err: Brackets must be paired');
      ...
      while (operationPriority[[stack[stack.length - 1]]] >= operationPriority[inputArr[i]])
        exprToRpn.push(stack.pop());
  ...
  for (let i = 0; i < exprToRpn.length; i++) {
    if (typeof exprToRpn[i] == 'number') stack.push(exprToRpn[i]);
      ...
      if (stack[stack.length - 1] === Infinity) throw new Error('Err: Division by zero.');
    }
  }
  return stack[0];
}
